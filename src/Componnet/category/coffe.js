...
const itemcoffe =document.getElementById("coffe-Item")

Product_coffe.map((p)=>{
    itemcoffe.innerHTML+=
    `
            <div class="w-[30%] bg-amber-50 h-[400px] rounded-2xl flex flex-col">
  <!-- Image -->
  <a href="../product-detaill/coffe/${p.link}" class="w-full h-[65%] block">
  ...
  <div class="w-full relative h-[35%] font-bold px-5 py-5 flex flex-col justify-between">
    ...
    <button 
      ...
      onclick="addToCart(${p.id})"><a href="../product-detaill/coffe/${p.link}">
      Chack
    </a></button>
  </div>
</div>
